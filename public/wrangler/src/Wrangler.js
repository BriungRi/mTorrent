...
const makeMongod = (filename, callback) => {
  console.log("Make MongoD");
  portscanner.findAPortNotInUse(3500, 4001, "127.0.0.1", function(error, port) {
    exec("mkdir -p /tmp/torr/" + port, (error, stdout, stderr) => {
      ...
      setTimeout(doneFun, 5000);
      try {
        mongodHelper.run();
      } catch (error) {
        console.log("mongod start err: " + error);
      }
    });
  });
};
        ...
        bucket
          ...
          .pipe(
            fs.createWriteStream(
              "/Users/" + os.userInfo().username + "/Downloads/" + replSetName
            )
          )
          .on("error", () => {
            console.log("error");
